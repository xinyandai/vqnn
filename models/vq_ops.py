...
class _LinearQuantize(object):
    ...
    def __call__(self, input, weight, tensor_):
        ...
        # escape gradient tracer
        weight.data = tensor
        return codes, nn.functional.linear(input, weight)
...
class _ConvQuantize(object):
    ...
    def __call__(self, input, weight, tensor_, bias=None,
...
class VQLinear(nn.Linear):
    def __init__(self, args, in_features, out_features, bias=True):
        ...
        self.quantize = _LinearQuantize(dim=args.dim, ks=args.ks)
        self.codes = None
        self.args = args
    ...
    def forward(self, input):
        ...
        self.codes, out = self.quantize(input, self.weight, self.weight.org)
...
class VQConv2d(nn.Conv2d):
    ...
    def __init__(self, args, in_channels, out_channels, kernel_size,
        ...
        self.quantize =  _ConvQuantize(dim=dim, ks=args.ks)
        self.codes = None
        self.args = args
    ...
    def forward(self, input):
        ...
        # quantize weight, decompress it to self.weight, while the codes
        # is saved and then calculate the linear multiplication result
        self.codes, out = self.quantize(
            input, self.weight, self.weight.org, None, self.stride,
